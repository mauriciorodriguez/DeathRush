...
//        Debug.Log("Hello!!!");
  ...
  //      Debug.Log("Hello!!!DC" + threadQuads.mrt.dirtyBlocks.Length);
        try
        {
...
//        if (thereSpecular)
  //      {
    ...
    //    }
        ...
        {
            if(Emission != null)Emission.Apply();
        }
...
//        }
    ...
    public void ApplyThreadQuad(ThraedQuad threadQuad)
    {
        ...
        if (threadQuad.ibEmission && asset3DCoat.ImportEmission) Emission.SetPixels(threadQuad.mrb.posX * QuadSize, threadQuad.mrb.posY * QuadSize, QuadSize, QuadSize, threadQuad.emissionQuad);
...
public class Asset3DCoat : ScriptableObject
{
    ...
    public bool SpecularSetupMode { get { return _SpecularSetupMode; } set { if (_SpecularSetupMode != value) { _SpecularSetupMode = value; if (InteractiveUpdate)updateMaterials(); } } }

    [SerializeField]
    bool _extractAO = true;
    public bool ExtractAO { get { return _extractAO; } set { if (_extractAO != value) { _extractAO = value; if (InteractiveUpdate)updateMaterials(); } } }

    [SerializeField]
    bool _extractCurvature = false;
    public bool ExtractCurvature { get { return _extractCurvature; } set { if (_extractCurvature != value) { _extractCurvature = value; if (InteractiveUpdate)updateMaterials(); } } }

    [SerializeField]
    bool _ImportEmission = true;
    public bool ImportEmission { get { return _ImportEmission; } set { if (_ImportEmission != value) { _ImportEmission = value; if (InteractiveUpdate)updateMaterials(); } } }

    [SerializeField]
    bool _HideAOInAlbedo = true;
    public bool HideAOInAlbedo { get { return _HideAOInAlbedo; } set { if (_HideAOInAlbedo != value) { _HideAOInAlbedo = value; if (InteractiveUpdate)updateMaterials(); } } }

    [SerializeField]
    bool _CreateNormalMap = true;
    public bool CreateNormalMap { get { return _CreateNormalMap; } set { if (_CreateNormalMap != value) { _CreateNormalMap = value; if (InteractiveUpdate)updateMaterials(); } } }

    [SerializeField]
    bool _CreateHeightMap = false;
    public bool CreateHeightMap { get { return _CreateHeightMap; } set { if (_CreateHeightMap != value) { _CreateHeightMap = value; if (InteractiveUpdate)updateMaterials(); } } }

    [SerializeField]
    bool _MargeAOAndMetallic = true;
    public bool MargeAOAndMetallic { get { return _MargeAOAndMetallic; } set { if (_MargeAOAndMetallic != value) { _MargeAOAndMetallic = value; if (InteractiveUpdate)updateMaterials(); } } }

    [SerializeField]
    bool _MargeHMAndMetallic = false;
    public bool MargeHMAndMetallic { get { return _MargeHMAndMetallic; } set { if (_MargeHMAndMetallic != value) { _MargeHMAndMetallic = value; if (InteractiveUpdate)updateMaterials(); } } }

    [SerializeField]
    float _NormalMapMultiply = 1.0f;
    public float NormalMapMultiply { get { return _NormalMapMultiply; } set { if (_NormalMapMultiply != value) { _NormalMapMultiply = value; if (InteractiveUpdate)updateMaterials(); } } }

    [SerializeField]
    float _HeightMapMultiply = 1.0f;
    public float HeightMapMultiply { get { return _HeightMapMultiply; } set { if (_HeightMapMultiply != value) { _HeightMapMultiply = value; if (InteractiveUpdate)updateMaterials(); } } }
...
/*	public void MakeMesh()
	{
		//
		int cnt = FacesInfo.Count;
		for(int i=0;i<cnt;i++){

			PlaneTriangulator plt;
			PMS.TriangulateFace(plt,i,false);
			FaceInfo* fi = &PMS.FacesInfo[i];
			if(i % 1000 == 0) BasicTool::ShowProgress(i,cnt,TextManager::GetText("Get mesh info"));
			for(int ti = 0; ti < plt.Result.GetAmount(); ti+=3){
				
				cvt.addTriangle(0, 0, 0, 0,
				                PMS.VertsUV[plt.Result[ti]].PosIndex,
				                PMS.VertsUV[plt.Result[ti]].NIndex,
				                plt.Result[ti],
				                PMS.VertsUV[plt.Result[ti + 1]].PosIndex,
				                PMS.VertsUV[plt.Result[ti + 1]].NIndex,
				                plt.Result[ti + 1],
				                PMS.VertsUV[plt.Result[ti + 2]].PosIndex,
				                PMS.VertsUV[plt.Result[ti + 2]].NIndex,
				                plt.Result[ti + 2]
				                );
...
*/
    ...
    {
/*        if (LayersInfoArray.Length != layersInfo.Count)
        {
            layersInfo.Clear();
            foreach (OneLayerInfo oli in LayersInfoArray)
            {
                layersInfo.Add(oli.LayerID, oli);
...
//                mrt.Bump.SetPixels32(mrt.bumpMap);
    ...
    public void updateAsset()
    {
        ...
        prefab.GetComponent<MeshRenderer>().sharedMaterials = mts;

        for (int i = prefab.transform.childCount-1; i >= 0; i--)
        {
            prefab.transform.GetChild(i).gameObject.GetComponent<MeshRenderer>().sharedMaterials = mts;
        }
...
//        Debug.Log("AssetPath: " + AssetPath);
 ...
 //       Debug.Log("prefabAvailable: " + prefabAvailable);
  //      Debug.Log("subObjects: " + SubObjects.Count);
        ...
        try
        {
            ...
            if (!JoinObjects)
            {
                ...
                {
                    GameObject subObj = new GameObject(SubObjects[i].Name);
        ...
        Mesh[] meshDatas = new Mesh[objects.Count];
        for (int i = 0; i < objects.Count; i++)
        {
            //
            if (objects[i].GetComponent<MeshRenderer>() == null) objects[i].AddComponent<MeshRenderer>();

            MeshFilter mfi = objects[i].GetComponent<MeshFilter>();
            if(mfi == null) mfi = objects[i].AddComponent<MeshFilter>();

            Mesh mesh = null;

            mesh = assetObject<Mesh>(SubObjects[i].Name);
            if (mesh == null)
            {
                mesh = new Mesh();
                mesh.name = "mesh";
                if (SubObjects.Length > i) mesh.name = SubObjects[i].Name;
//                AssetDatabase.AddObjectToAsset(mesh, prefab);
            }

            mesh.subMeshCount = RTS.Length;
//            mfi.sharedMesh = mesh;
  //          mfi.mesh = mesh;
            meshDatas[i] = mesh;
        }

//            for (int oi = 1; oi < SubObjects.Count; oi++)
  //          {
    //            if (assetObject<GameObject>(SubObjects[oi].Name) != null)
      //          {
        //            GameObject ngo = new GameObject(SubObjects[oi].Name);
          //          ngo.transform.parent = prefab.transform;
            //    }
           // }

//            MeshFilter mf = prefab.GetComponent<MeshFilter>();
  //          if (mf == null) mf = prefab.AddComponent<MeshFilter>();
            ...
            List<int>[,] iTriangles = new List<int>[SubObjects.Length, RTS.Length];
            for (int iso = 0; iso < SubObjects.Length; iso++)
                for (int i = 0; i < RTS.Length; i++)
                {
                    iTriangles[iso, i] = new List<int>();
                }

            int faceId = (int)Faces[0].Key;

            for (int i = 0; i < Faces.Length; i++) if (Faces[i].Key < FacesInfo.Length)
            {

                if (faceId != Faces[i].Key)
                {
                    int objId = (int)FacesInfo[faceId].ObjID;
                    if (JoinObjects) objId = 0;
  //                  int mtlId = (int)FacesInfo[faceId].MtlID;
                    int uvIdx = (int)FacesInfo[faceId].uvIdx;
                    //                Debug.Log("objId: " + objId);
                    pl.Triangulate(true);

                    //				Debug.Log("faceVuvs"+faceVuvs.Count+" res"+pl.Result.Count);
                    for (int vid = 0; (float)vid < Math.Truncate(pl.Result.Count / 3.0) * 3; vid++)
                    {
                        iTriangles[objId, uvIdx].Add((int)faceVuvs[pl.Result[vid]]);
                    }
                    //				iTriangles.Add((int)faceVuvs[0]);
                    //				iTriangles.Add((int)faceVuvs[1]);
                    //				iTriangles.Add((int)faceVuvs[2]);
                ...
                //			Debug.Log("Face: "+Faces[i].Key+" Vert: "+Faces[i].Value+" Pos: "+iPos);

            }

            pl.Triangulate(true);
            if (faceId < FacesInfo.Length)
            {
                int iobjId = (int)FacesInfo[faceId].ObjID;
                if (JoinObjects) iobjId = 0;
                //            int imtlId = (int)FacesInfo[faceId].MtlID;
                int iuvIdx = (int)FacesInfo[faceId].uvIdx;

                //		Debug.Log("faceVuvs"+faceVuvs.Count+" res"+pl.Result.Count);
                for (int vid = 0; (float)vid < Math.Truncate(pl.Result.Count / 3.0) * 3; vid++)
                {
                    iTriangles[iobjId, iuvIdx].Add((int)faceVuvs[pl.Result[vid]]);
                }
            }
            ...
            {
                verts[i] = Verts[(int)VertsUV[i].PosIndex].Pos;
                verts[i].x *= -1.0f;
...
//                MeshFilter mfi = objects[i].GetComponent<MeshFilter>();
                meshDatas[i].vertices = verts;
                meshDatas[i].normals = norms;
                meshDatas[i].uv = uvs;
                meshDatas[i].tangents = tangets;
                //     meshDatas[i].uv2 = uvs2;
            }


            int objCnt = 1;
            if (!JoinObjects) objCnt = SubObjects.Length;
       //     Debug.Log("objCnt: " + objCnt);
            for (int iso = 0; iso < objCnt; iso++)
            {

                for (int irts = 0; irts < RTS.Length; irts++)
                {
                    int tcnt = iTriangles[iso, irts].Count;
                    //                Debug.Log("iso: " + iso + " rts: " + irts + " tCnt: " + iTriangles[iso, irts].Count);
                    int[] tidxs = new int[tcnt];

                    for (int itt = 0; itt < tcnt; itt++)
                    {
                        tidxs[itt] = iTriangles[iso, irts]/*[itt];*/[tcnt - itt - 1];
 //                       tidxs[itt] = iTriangles[iso, irts][itt];
                    }
                    meshDatas[iso].SetTriangles(tidxs, irts);

                }
                MeshFilter mfi = objects[iso].GetComponent<MeshFilter>();
                mfi.sharedMesh = meshDatas[iso];
                mfi.mesh = meshDatas[iso];
            }


//            mf.sharedMesh = mesh;
  //          mf.mesh = mesh;
//            if (prefab.GetComponent<MeshRenderer>() == null) prefab.AddComponent<MeshRenderer>();
            ...
            for (int i = 0; i < objects.Count; i++)
            {
                if (assetObject<Mesh>(meshDatas[i].name) == null) AssetDatabase.AddObjectToAsset(meshDatas[i], prefab);
                KeepAssets.Add(meshDatas[i]);
                if (i == 0)
                {
                    MeshFilter mfi = prefab.GetComponent<MeshFilter>();
                    mfi.sharedMesh = meshDatas[i];
                    mfi.mesh = meshDatas[i];
                }
                else
                {
                    MeshFilter mfi = prefab.transform.GetChild(i-1).gameObject.GetComponent<MeshFilter>();
                    mfi.sharedMesh = meshDatas[i];
                    mfi.mesh = meshDatas[i];
                }
            }

/*            if (prefab.GetComponent<Link3DCoat2>() == null)
            {
                prefab.AddComponent<Link3DCoat2>();

            }
            prefab.GetComponent<Link3DCoat2>().asset3DCoat = this;
            */
...
public class meshMapRenderer
{
    ...
    public bool DrawMRT(MRenderTarget amrt){
        Debug.Log("Draw alpha map");

        mrt = amrt;
        glm = mrt.asset3DCoat;
        mesh = glm.prefab.GetComponent<MeshFilter>().sharedMesh;

        bool objThereAlpha = false;
///        Material[] materials = glm.prefab.GetComponent<MeshRenderer>().sharedMaterials;
   ///     Material[] materials2 = new Material[mrt.asset3DCoat.RTS.Count * 2];
  //      materials.CopyTo(materials2,0);
      //  glm.prefab.GetComponent<MeshRenderer>().sharedMaterials = materials2;
       // glm.prefab.GetComponent<MeshRenderer>().materials = materials2;

        //        wnormMap = new Color32[mrt.rtSizeX*mrt.rtSizeY];
   //     wnormMap = new Color32[mrt.rtSizeX*mrt.rtSizeY];
     //   wnormMap = mrt.Albedo.GetPixels32();

        meshMask = new byte[mrt.rtSizeX * mrt.rtSizeY];
 //       for (int i = 0; i < meshMask.Length; i++) meshMask[i] = 0;
         albedoMap = mrt.Albedo.GetPixels32();
        int UVID = 0;
        for (int i = 0; i < mrt.asset3DCoat.RTS.Length; i++) if (mrt.asset3DCoat.RTS[i] == mrt) UVID = i; 
   //     Debug.Log("UVAD"+UVID);
        uv = mesh.uv;
        normals = mesh.normals;
        //      for (int iobj = 0; iobj < mrt.asset3DCoat.objects.Count; iobj++)
    //    {
            mesh.subMeshCount = mrt.asset3DCoat.RTS.Length * 2;
            ...
            for (int tid = 0; tid < tcnt; tid++)
            {
                ...
                if (thereAlpha) objThereAlpha = true;
                //                bool thereAlpha = DrawTriangle(triangles[tid*3], triangles[tid*3+1], triangles[tid*3+2], 0);
                DrawMeshMask(triangles[tid*3], triangles[tid*3+1], triangles[tid*3+2], 0);
                if (thereAlpha)
                {
                    nTri2.Add(triangles[tid * 3]);
                    nTri2.Add(triangles[tid * 3 + 1]);
                    nTri2.Add(triangles[tid * 3 + 2]);
                }
                else
                {
                    nTri1.Add(triangles[tid * 3]);
                    nTri1.Add(triangles[tid * 3 + 1]);
                    nTri1.Add(triangles[tid * 3 + 2]);
                }
            }



            mesh.SetTriangles(nTri1.ToArray(), UVID);
            mesh.SetTriangles(nTri2.ToArray(), mrt.asset3DCoat.RTS.Length + UVID);

            mrt.meshMask = meshMask;

     //       mrt.Albedo.SetPixels32(wnormMap);
       //     mrt.Albedo.Apply();
    ...
    void DrawMeshMask(int iP1, int iP2, int iP3, int threadId)
    {
        ...
        Vector2 trianglep1 = new Vector2(uv[iP1].x*fTxtWidth, (1.0f - uv[iP1].y)*fTxtHeight);
        Vector2 trianglep2 = new Vector2(uv[iP2].x*fTxtWidth, (1.0f - uv[iP2].y)*fTxtHeight);
        Vector2 trianglep3 = new Vector2(uv[iP3].x*fTxtWidth, (1.0f - uv[iP3].y)*fTxtHeight);
 ...
 //               wnormMap[idx].r = 255;
    ...
    bool DrawTriangle(int iP1, int iP2, int iP3, int threadId)
    {
        ...
        Vector3 cNorm = (normals[iP1] + normals[iP2] + normals[iP3]) / 3.0f;
        ...
        for (int iy = boundsMinY + 1; iy < boundsMaxY - 1; ++iy)
        {
            ...
            for (int ix = xFrom; ix <= xTo; ix++)
            {
                ...
                Vector3 vnorm = (normals[iP1] * ve3
                        + normals[iP2] * ippX
                        + normals[iP3] * ippY);///2.0f+halfV3;
    ...
    bool getThereAlpha(int iP1, int iP2, int iP3, int threadId)
    {
        ...
        Vector2 trianglep1 = new Vector2(uv[iP1].x * fTxtWidth, uv[iP1].y * fTxtHeight);
        Vector2 trianglep2 = new Vector2(uv[iP2].x * fTxtWidth, uv[iP2].y * fTxtHeight);
        Vector2 trianglep3 = new Vector2(uv[iP3].x * fTxtWidth, uv[iP3].y * fTxtHeight);
