...
public class VehicleIAController : Vehicle
{

    public float firstUpGearValor;
    public float maximunGearUpValor;
    public float minimunGearValor;
    private float _normalGear;
    ...
    protected override void Start()
    {
       ...
       /* _maxHp = K.IA_MAX_HP;
        ...
        _normalGear = vehicleVars.topSpeed;
        _normalForce = vehicleVars.torque; 
    }
    ...
    {
        base.ResetCar();
        if(_nextCheckpoint != null) SetNextCheckpoint(_nextCheckpoint, _nextCheckpoint.minRandom, _nextCheckpoint.maxRandom);
    }
    ...
    {
        //Comentar en caso de colisiones extraordinarias
        base.OnCollisionEnter(hit);

        /*if (hit.gameObject.layer == K.LAYER_PLAYER)
        {
            Debug.LogError("Impacto entre el jugador y una IA");
            hit.gameObject.GetComponentInParent<Rigidbody>().AddExplosionForce(100f, hit.contacts[0].point, 40f, 2f, ForceMode.Impulse);
            this.gameObject.GetComponentInParent<Rigidbody>().AddExplosionForce(100f, hit.contacts[0].point, 40f, 2f, ForceMode.Impulse);
        }*/
    ...
    public void ChangeGear(string id)
    {
        ...
        {
            case "normal":
                vehicleVars.topSpeed = _normalGear;
                vehicleVars.torque = _normalForce;
                break;
            case "high":
                vehicleVars.topSpeed = _normalGear + firstUpGearValor;
                vehicleVars.torque = _normalForce + firstUpGearValor;
                break;
            case "low":
                vehicleVars.topSpeed = _normalGear - minimunGearValor;
                vehicleVars.torque = _normalForce - minimunGearValor;
                break;

            case "mach3":
                vehicleVars.topSpeed = _normalGear + maximunGearUpValor;
                vehicleVars.torque = _normalForce + maximunGearUpValor;
                break;
            default:
                print("Error de Id");
                break;
