...
public class PostGameManager : MonoBehaviour {
    ...
    void Awake ()
    {
        _ID = PlayerPrefs.GetInt("CurrentID");
    ...
    public void Next()
    {
        ...
        {
            int sum = PlayerPrefs.GetInt(traitSelected.GetComponent<TraitSelected>().trait+_ID) + 1;
            if (sum > 5)
            {
                sum = 5;
            }
            PlayerPrefs.SetInt(traitSelected.GetComponent<TraitSelected>().trait+_ID, sum);
            SceneManager.LoadScene((int)SCENES_NUMBER.HUB);
        }
